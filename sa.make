...
results/prostate/cesource: OPT =  --target_losses="$(L_OR)" --target_dataset "data/SB" \
	     --network UNet --batch_size 8 --model_weights="" --lr_decay 1 \
	    
# full supervision
results/prostate/fs: OPT =  --target_losses="$(L_OR)" \
	     --network UNet --batch_size 8 --model_weights="$(M_WEIGHTS_uce)" --lr_decay 1 \

# SFDA. Put --saveim False and remove --entmap and --do_hd 90 to speed up
results/prostate/sfda: OPT = --target_losses="[('EntKLProp2', {'inv_consloss':True,'lamb_se':1,'lamb_conspred':1, 'lamb_consprior':1,'ivd':True,'weights_se':[0.1,0.9],'idc_c': [1],'curi':True,'power': 1},'PredictionBounds', \
      {'margin':0,'dir':'high','idc':[0,1],'predcol':'dumbpredwtags','power': 1, 'mode':'percentage','sizefile':'sizes/prostate_sa.csv'},'norm_soft_size',1)]" \
...
#inference mode : saves the segmentation masks for a specific model saved as pkl file:
