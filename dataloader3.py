...
from utils import one_hot, depth, read_nii_image, pad_to
...
def png_transform_t(resolution: Tuple[float, ...], K: int) -> Callable[[D], Tensor]:
    return transforms.Compose([
        lambda img: np.array(img)[np.newaxis, ...],
        lambda nd: pad_to(nd, 512,512),
        lambda nd: (nd /255),  # max <= 1
        lambda nd: torch.tensor(nd, dtype=torch.float32),
    ])

def nii_transform_t(resolution: Tuple[float, ...], K: int) -> Callable[[D], Tensor]:
    return transforms.Compose([
        lambda img : np.array(img),
        lambda nd: pad_to(nd, 512,512),
        lambda nd: (nd /3896),  # max <= 1
        lambda nd: torch.tensor(nd, dtype=torch.float32),
    ])



def nii_transform2(resolution: Tuple[float, ...], K: int) -> Callable[[D], Tensor]:
    return transforms.Compose([
        lambda nd: ((nd+4) / 8),  # max <= 1
        lambda nd: torch.tensor(nd, dtype=torch.float32),
        lambda nd: torch.tensor(nd, dtype=torch.int64)[None, ...],  # Add one dimension to simulate batch
        #lambda nd: print(nd.shape),
        #lambda nd : nd.permute(0, 3, 1, 2),
        #lambda nd : nd[0,:,:],
        #lambda nd: print(nd.shape)
    ])
...
def nii_gt_transform(resolution: Tuple[float, ...], K: int) -> Callable[[D], Tensor]:
    return transforms.Compose([
        #lambda img: np.concatenate((img,img,img), axis=0),
        #lambda img: np.repeat(img[np.newaxis,:, :], 3, axis=0), 
        lambda nd: torch.tensor(nd, dtype=torch.int64) ,  # need to Add one dimension to simulate batch
        partial(class2one_hot, C=K),
...
def laplacian_transform(resolution: Tuple[float, ...], K: int) -> Callable[[D], Tensor]:
    return transforms.Compose([
        lambda img: img.convert('L'),
        lambda img: np.array(img)[np.newaxis, ...],
        lambda nd: nd / 255,  # max <= 1
        lambda nd: torch.tensor(nd, dtype=torch.float32),
        lambda t: utils.laplacian(t[0], sigma=100, eps=1e-10)
    ])
...
class SliceDataset(Dataset):
    ...
    def __getitem__(self, index: int) -> Dict[str, Union[str,
        ...
        if path_name.suffix == ".png":
            ...
            images = [read_nii_image(files[index]) for files in self.files]
        else:
        ...
        if self.dimensions == 2:  # Everything fits within one patch
            ...
            patches = [tuple([slice(e,
                                    min(e + patch_size,
                                        m))  # this way we avoid overflow
                              for (e, m) in zip(origin, img_shape)
                              ])
                       for origin in zip(bxs.flatten(), bys.flatten(), bzs.flatten())]
            # Now discard the last small slices that are barely the overlap width:
            patches = [patch for patch in patches if all((s.stop - s.start) > 12 for s in patch)]
            # shuffle(patches)  # Disabled during debug
        else:
...
class PatientSampler(Sampler):
    def __init__(self, dataset: SliceDataset, grp_regex, shuffle=False, quiet=False) -> None:
        ...
        self.shuffle_fn: Callable = (lambda x: sample(x, len(x))) if self.shuffle else id_
